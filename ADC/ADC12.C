...
* 说    明：此程序可以完成AD通用的初始化，这里选用的时钟是
            ...
            本函数，改变内部参考电压的值或关闭之)。
*********************************************************/
char ADC12Init(char n,char channels[],char repeat)
{
    if(n>15)
        return 0;
    //SHT0_0 
    ADC12CTL0 = ADC12ON + MSC + SHT0_0 + REFON + REF2_5V;          // 开启ad,n=1,内部参考电压2.5v
    ADC12CTL1 = SHP + ADC12SSEL_3;                                 // 使用采样定时器，SMCLK时钟源，Use sampling timer, SMCLK
    
    for(int i = 0;i < n;i++)
    {
        if(channels[i] >= 0x80)
            return 0;
        *(char*)(ADC12MCTL0 + i) = channels[i];                   //将每个通道放入每个MCTL
    }
    *(char*)(ADC12MCTL0 + n - 1) |= EOS;                          //最后一个通道时，序列结束
    
    if(repeat != 0)                                                  //多次转换
    {
        ADC12CTL1 |= CONSEQ_3;                                    //选择序列通道多次转换模式
    }
    else
    {
        ADC12CTL1 |= CONSEQ_1;                                    //选择序列通道单次转换模式
    }
    
    ADC12IE = 1<<(n-1);                                           // ADC12IFG.n-1处中断  （十六进制中左移n-1位）
    return 1;
}
...
* 说    明：每次当多个通道转换完成后，打开中断时，单片机会
            ...
            掉现在的函数体。）
*********************************************************/
#pragma vector=ADC_VECTOR                                         //一般是转换结束后的结果
__interrupt void ADC12ISR (void)
{
    static int i;
    results[0][i] = ADC12MEM0;                                    // 将转换结果MEMx存入数据中Move results, IFG is cleared
    results[1][i] = ADC12MEM1;                                    // Move results, IFG is cleared
    results[2][i] = ADC12MEM2;                                    // Move results, IFG is cleared
    i++;
    if(i>31)                                                      //多次转换时 转换次数大于数组范围则停止转换
    {
        //多次重复采样时，在这里方处理函数
        ADC12CTL0 &=~ ENC;                                        //停止转换
        i=0;
